...
{
     fprintf(stderr, "ENTER LoadInternalFileToMemory ckey=%02x%02x%02x%02x\n",
             pCKeyEntry->CKey[0],
             pCKeyEntry->CKey[1],
             pCKeyEntry->CKey[2],
             pCKeyEntry->CKey[3]); fflush(stderr);
    HANDLE hFile = NULL;
    ...
    if(OpenFileByCKeyEntry(hs, pCKeyEntry, CASC_STRICT_DATA_CHECK, &hFile))
    {
        ...
        if(dwErrCode == ERROR_SUCCESS)
        {
            ...
            if((dwErrCode = FileData.SetSize(cbFileData)) == ERROR_SUCCESS)
            {
                ...
                fprintf(stderr, "PRE READ: size=%u\n", cbFileData); fflush(stderr);
                CascReadFile(hFile, FileData.pbData, cbFileData, &dwBytesRead);
                fprintf(stderr, "POST READ bytes=%u\n", dwBytesRead); fflush(stderr);
                fprintf(stderr, "READ INTERNAL: expected=%u read=%u ckey=%02x%02x%02x%02x\n", cbFileData, dwBytesRead, pCKeyEntry->CKey[0], pCKeyEntry->CKey[1], pCKeyEntry->CKey[2], pCKeyEntry->CKey[3]); fflush(stderr);
